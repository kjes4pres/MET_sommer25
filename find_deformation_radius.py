...
def bvf_calc(mfile):
    ...
    zlevs = np.arange(-3000, -520, 20)
    zlevs = np.insert(zlevs, len(zlevs), values=np.arange(-300, -105, 5), axis=0)
    zlevs = np.insert(zlevs, len(zlevs), values=np.arange(-100, -52, 2), axis=0)
    zlevs = np.insert(zlevs, len(zlevs), values=np.arange(-50, 1, 1), axis=0)
    ...
    outputf = '/home/kjsta7412/sommer_25/MET_sommer25/output_bdr/' + mfile.split('/')[-1].replace('.nc','NEWTRYZLEVS.nc')
