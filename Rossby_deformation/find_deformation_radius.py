...
interpolating salinity and temperature from s-levels to z-depths,
calculating the potential density on z-depths, 
...
def bvf_calc(mfile):
    ...
    z_r = np.insert(z_r, 0, values=-1.0 * grid.h, axis=0)
    ...
    # Temporary array holding integrated buoyancy frequency
    tmpn = (np.ones([1, salt.shape[2], salt.shape[3]]) * np.nan)
    # Temporary array holding Rossby radius
    tmpr = (np.ones([1, salt.shape[2], salt.shape[3]]) * np.nan)

    # Making a filepath to write to
    ...
    gamma_r = rootgrp.createVariable(
        "gamma_r",
        "f8",
        (
            "ocean_time",
    ...
    for y in range(0, salt.shape[2]):
        for x in range(0, salt.shape[3]):
            ...
            tmpnz = zlevs[
                np.where(zlevs[:] > z_r[:, y, x].min())
            ].squeeze()  # z-levels to calculate N on
            N_depth = (tmpnz[0:-1] + tmpnz[1:]) / 2.0  # Midpoints between z-levels

            if 1:
                t = -1
                # Adding first and last value to each end of the array to match length of z-levels (tmpnz)
                tmpnS = salt[t, :, y, x]
                tmpnS = np.append(tmpnS, tmpnS[-1])
                tmpnS = np.insert(tmpnS, 0, values=tmpnS[0], axis=0)
                tmpnT = temp[t, :, y, x]
                tmpnT = np.append(tmpnT, tmpnT[-1])
                tmpnT = np.insert(tmpnT, 0, values=tmpnT[0], axis=0)
                ...
                n2 = N2(densm, tmpnz)
                n2[np.where(n2 < 0)] = 0.0000001
                N = np.sqrt(n2).squeeze()
